...
async function runFeatureTests() {
    ...
    try {
        ...
        if (stats1.subscriptionsCount !== 0 ||
        ...
        const adminToken = jwt.sign({ id: userId, email: testEmail, role: 'ADMIN' }, 'super-secret-texxxnopor-key', { expiresIn: '1h' });
        ...
        if (stats2.subscriptionsCount !== 1 ||
        ...
        const searchCatRes = await fetch(`http://localhost:${PORT}/api/videos?category=Amateur`);
        ...
        const searchTagRes = await fetch(`http://localhost:${PORT}/api/videos?tag=pareja`);
