...
async function runTests() {
  ...
  try {
    ...
    const createActor = await request(
      '/api/admin/actors',
      'POST',
      {
    ...
    const editActor = await request(
      `/api/admin/actors/${actorId}`,
      'PUT',
      {
    ...
    const updateRole = await request(
      `/api/admin/users/${secondUser.data?.user?.id}/role`,
      'PATCH',
      { role: 'CREATOR' },
      adminToken
    );
